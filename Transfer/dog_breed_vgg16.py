...
def print_error_check(train_data, test_data, val_data):
    ...
    :param test_data-> Testing Data Loader
...
def load_data_vgg(train_dir, test_dir, val_dir, batch_size=20, error_check=0, input_size=224):
    """
    Loads data from specified path to tensors.
    inputs:
    :param val_dir: Path for validation images
    :param train_dir: Path for training images
    :param test_dir: Path for testing images
    :param batch_size: Images to be processed in one batch, defaulted to 20
    :param error_check: if this flag is set, the images from directories will be checked for error.
    :param error_check: Prints warning in case of load error
    :param input_size: Size of the CNN input image, defaulted to 224 for vgg16
    :returns Training , Testing and Validation Data loaders
    """
    input_size = 224
    num_workers = 0
    data_transform = transforms.Compose([transforms.RandomResizedCrop(input_size),
                                         transforms.ToTensor()])
...
def get_transfer_model():
    """Returns a vgg16 model, with output tuned for dog breed classification
    :param: nil
    :returns vgg_16: vgg16 model tuned for dog breed classification"""
    vgg_16 = models.vgg16(pretrained=True)
    print(vgg_16.classifier[6].in_features)
    print(vgg_16.classifier[6].out_features)
    in_size = vgg_16.classifier[6].in_features
    out_size = class_size
    for parm in vgg_16.features.parameters():
        parm.requires_grad = False
    dog_classifier = nn.Sequential(OrderedDict([('fc6', nn.Linear(in_size, out_size))]))
    vgg_16.classifier[6] = dog_classifier
    return vgg_16
...
def train_network_vgg(train_data_loader, val_data_loader, model, device, criterion, optimizer, epochs=20):
...
def load_transfer_model(path, model):
    """Loads the model with saved state dict
